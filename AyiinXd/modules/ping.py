...
async def _(ping):
    ...
    await Ayiin.edit("**𖦻𖦻**")
    await Ayiin.edit("**𖦻𖦻𖦻**")
    await Ayiin.edit("**𖦻𖦻𖦻𖦻**")
    await Ayiin.edit("**𖦻𖦻𖦻𖦻𖦻**")
    end = datetime.now()
    ...
    sleep(3)
    await Ayiin.edit(
        f"**𖦻 𖦻ʙᴏᴄɪʟ-ᴜꜱᴇʀʙᴏᴛ𖦻 𖦻**\n\n"
        f"𖦻 **𝙿𝙸𝙽𝙶𝙴𝚁 :** `%sms`\n"
        f"𖦻 **𝚄𝙿𝚃𝙸𝙼𝙴 :** `{uptime}` \n"
        f"𖦻 **𝙾𝚆𝙽𝙴𝚁 :** [{user.first_name}](tg://user?id={user.id})" % (duration)
    )
